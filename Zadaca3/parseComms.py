...
def _parse_command(self, line, b, c):
    if line[0] == "@":
        ...
        if addr.isdigit():
            num = int(addr)
            if num < 0 or num > 32767:  # provjera je li adresa u rasponu 15bitnog broja
                self._flag = False
                self._line = c
                self._errm = f"Invalid memory address: {addr}"
                return ""
            return "{0:016b}".format(num)
        else:
            self._flag = False
            self._line = c
            self._errm = f"Invalid symbol or address: {addr}"
            return ""
    else:
        op = ""
        dest = ""
